...
class ReservationViewController: UIViewController {
    ...
    // viewWillAppear executes every time the user goes to the MyReservation page
    override func viewWillAppear(_ animated: Bool) {
        ...
        for i in 0..<currentUDList.count {
            // Add to MyReservation page only if event is reserved, and it does not already exist in the reservations list
            if (currentUDList[i].isReserved && !reservationsList.contains(where: {$0.title == currentUDList[i].title})) {
...
extension ReservationViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        // Return number of reservations in array
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Dictionary to store event information to be cancelled
    ...
    @objc func cancelEvent(_ sender: UIButton) {
        ...
        // Update event User Defaults using information from dictionary, set reservation status and number of tickets
